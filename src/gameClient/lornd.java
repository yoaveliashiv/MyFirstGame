...
public class lornd {
	...
	public static void test1() {
		...
		try {
			...
			for (int j = 0; j < E.length; j++) {
				...
				System.out.println(E[j].getSrc()+"start");
			}
	...
	trehd(game);
		while(game.isRunning()) {
			moveRobots(game, gg,roobet ,window,dests,new int[E.length]);
		}
	 ...
	 * @param log
	 */
	private static void moveRobots(game_service game, DGraph gg, ArrayList<Nodedata> roobet, Window window,int []dests, int[] ks) {
		
	//	for (int j = 0; j < dests.length; j++) System.out.println("src:"+ks[j]);
		...
		if(log!=null) {
			...
			for(int i=0;i<log.size();i++) {
				...
				try {
					...
					if(dest==-1) {	
						for (int j = 0; j < roobet.size(); j++) {
							
							
							if(dest1.size()==0&&dests[0]==-1) {
								
							AlgoGameWhereTheRoobotsGo R=new AlgoGameWhereTheRoobotsGo(game,ks);
							dest1=R.getDest1();
							//System.out.println(dest1.size()+"size");
							}
							if(dests[0]==-1)	dest=dest1.get(0).getKey();
							else dest=dests[j];
							System.out.println("id:"+j+"src:"+ks[j]+"dest:"+dest);
							ks[j]=dest;
							if(dests[0]==-1)dest1.remove(0);
